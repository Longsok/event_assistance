...
<body class="antialiased overflow-x-hidden" style="background:#080b14;color:#e2e8f0">
                ...
                <div class="flex items-center gap-3">
                    @auth
                        @if(auth()->user()->hasRole('admin'))
                            <a href="{{ route('admin.dashboard') }}" class="text-sm text-slate-300 hover:text-white transition px-4 py-2 rounded-lg">Admin Panel</a>
                        @else
                            <a href="{{ route('dashboard') }}" class="text-sm text-slate-300 hover:text-white transition px-4 py-2 rounded-lg">Dashboard</a>
                        @endif
                    @else
                        @if(Route::has('login'))
                        ...
                        @if(Route::has('register'))
                            <a href="{{ route('register') }}" class="btn-primary text-sm text-white font-medium px-5 py-2 rounded-xl">Get started free</a>
                        @endif
                    @endauth
                </div>
            </div>
    ...
    <section class="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            $features = [
                ...
                ['title'=>'Budget Tracking',   'desc'=>'Keep finances in check with real-time budget vs. actual spending across all categories.','icon'=>'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z','accent'=>'#10b981','glow'=>'rgba(16,185,129,.12)'],
    ...
    <footer class="relative z-10 border-t px-4 py-6 text-center text-xs"
            style="border-color:rgba(255,255,255,.08);color:#94a3b8">
...
Write-Host "Done. Run: php artisan view:clear" -ForegroundColor Green
