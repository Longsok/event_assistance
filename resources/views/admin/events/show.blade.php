<x-admin-layout title="Event Detail">
    ...
    <div class="rounded-xl border border-gray-800 p-6 space-y-4" style="background:#111827">
        <div class="flex items-start justify-between">
            <div>
                <h2 class="text-xl font-bold text-white">{{ $event->title }}</h2>
                <p class="text-gray-400 text-sm mt-1">{{ $event->user->name ?? '-' }} &middot; {{ $event->category->name ?? '-' }}</p>
            </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t border-gray-800">
