...
<div class="py-6 px-4 sm:px-6 max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold mt-1" style="color:var(--text-strong)">Scan Guest QR</h2>
        <p class="text-sm mt-0.5" style="color:var(--text-soft)">{{ $event->title }} — Point camera at guest's invite QR code</p>
    </div>

    {{-- Stats --}}
    <div class="rounded-2xl border p-4 mb-5" style="background:var(--panel);border-color:var(--border)" id="stats-bar">
        <div class="flex items-center justify-between">
            <div class="text-center">
                <p class="text-2xl font-bold text-indigo-400" id="checked-count">—</p>
                <p class="text-xs mt-0.5" style="color:var(--text-soft)">Checked In</p>
            </div>
            <div class="text-center">
                <p class="text-2xl font-bold" style="color:var(--text-strong)" id="total-count">—</p>
                <p class="text-xs mt-0.5" style="color:var(--text-soft)">Total Guests</p>
            </div>
            <div class="text-center">
                <p class="text-2xl font-bold text-amber-400" id="remaining-count">—</p>
                <p class="text-xs mt-0.5" style="color:var(--text-soft)">Remaining</p>
            </div>
        </div>
    </div>

    {{-- Camera scanner --}}
    <div class="rounded-2xl border overflow-hidden mb-5" style="background:var(--panel);border-color:var(--border)">
        <div class="p-4 flex items-center justify-between" style="border-bottom:1px solid var(--border)">
            <p class="font-semibold" style="color:var(--text-strong)">Camera Scanner</p>
        ...
        <div id="camera-placeholder" class="flex items-center justify-center" style="min-height:280px">
            <div class="text-center">
                ...
                <p class="text-sm" style="color:var(--text-soft)">Click "Start Camera" to begin scanning</p>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-2xl border p-5 mb-5" style="background:var(--panel);border-color:var(--border)">
        <h3 class="font-semibold mb-4" style="color:var(--text-strong)">Manual Entry</h3>
        <div class="flex gap-3">
            <input type="text" id="manual-code" placeholder="Enter guest code..."
                   class="flex-1 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 uppercase"
    ...
    <div id="result-box" class="hidden rounded-2xl p-5 mb-5 border">
        <div class="flex items-center gap-4">
            ...
            <div>
                <p id="result-name" class="text-xl font-bold" style="color:var(--text-strong)"></p>
    ...
    <div class="rounded-2xl border overflow-hidden" style="background:var(--panel);border-color:var(--border)">
        <div class="px-5 py-4" style="border-bottom:1px solid var(--border)">
            <p class="font-semibold" style="color:var(--text-strong)">Recent Check-ins</p>
        </div>
        <div id="recent-checkins">
            <div class="px-5 py-8 text-center text-sm" style="color:var(--text-soft)">No check-ins yet</div>
        </div>
    </div>
</div>
...
function processCode(code) {
    fetch('{{ route('events.attendance.scan.process', $event) }}', {
        ...
        if (!r.ok) {
            showResult('error','✗','Error', data.message || ('Server error ' + r.status));
            return;
        }
        ...
        loadStats();
    }).catch(err => showResult('error','✗','Error', err.message || 'Connection error'));
}
...
function addRecent(name, time) {
    ...
    d.style.cssText = 'display:flex;align-items:center;justify-content:space-between;padding:12px 20px;border-bottom:1px solid var(--border-soft)';
    d.innerHTML = `<div style="display:flex;align-items:center;gap:10px"><div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,#059669,#047857);display:flex;align-items:center;justify-content:center;color:white;font-size:12px;font-weight:700">${name.charAt(0).toUpperCase()}</div><p style="color:var(--text-strong);font-size:14px">${name}</p></div><span style="font-size:12px;color:var(--text-soft)">${time}</span>`;
