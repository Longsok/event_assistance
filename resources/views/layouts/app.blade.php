...
<head>
    ...
    <style>
        *{font-family:'Outfit',sans-serif}
        .nav-active{background:linear-gradient(135deg,#4f46e5,#7c3aed);color:#fff}
...
<body class="antialiased bg-slate-50 text-slate-900 overflow-hidden" style="height:100vh">
<div class="flex overflow-hidden" style="height:100vh" x-data="{mob:false,prof:false}">

    <div x-show="mob" @click="mob=false" class="fixed inset-0 bg-black/40 z-20 lg:hidden"
    ...
    <aside class="fixed inset-y-0 left-0 w-64 bg-white border-r border-slate-200 z-30 flex flex-col overflow-hidden
        ...
        <div class="h-16 flex items-center px-5 border-b border-slate-100 flex-shrink-0">
            <div>
                <p class="font-semibold text-slate-900 text-sm leading-tight">{{ config('app.name') }}</p>
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            ...
            <a href="{{ route($l['route']) }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition {{ $a?'nav-active':'nav-inactive' }}">
            ...
            @endforeach
            <div class="pt-3 pb-1"><div class="h-px bg-slate-100"></div></div>
            <a href="{{ route('events.create') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition text-indigo-600 bg-indigo-50 border border-indigo-200 hover:bg-indigo-100">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="border-t border-slate-100 p-3 flex-shrink-0">
            <div class="relative">
                <button @click="prof=!prof" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50 transition text-left">
                    <div class="w-8 h-8 rounded-xl text-xs font-bold text-white flex items-center justify-center flex-shrink-0" style="background:linear-gradient(135deg,#4f46e5,#7c3aed)">
                    ...
                    <svg class="w-4 h-4 text-slate-400 flex-shrink-0 transition-transform" :class="prof?'rotate-180':''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden" style="height:100vh">
        <header class="h-16 bg-white border-b border-slate-200 sticky top-0 z-10 flex items-center gap-4 px-4 lg:px-6 flex-shrink-0">
            ...
            <div class="w-8 h-8 rounded-xl text-xs font-bold text-white flex items-center justify-center" style="background:linear-gradient(135deg,#4f46e5,#7c3aed)">
                {{ strtoupper(substr(Auth::user()->name,0,1)) }}
            </div>
        </header>
