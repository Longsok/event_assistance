...
<div class="py-6 px-4 sm:px-6 lg:px-8 max-w-3xl mx-auto">
    <div class="mb-8">
        ...
        <h2 class="text-2xl font-bold" style="color:var(--text-strong)">Edit Event</h2>
    </div>
    ...
    <style>
        ...
        .dark-input option { background:var(--panel);color:var(--text); }
    </style>
    ...
    <form method="POST" action="{{ route('events.update', $event) }}" class="space-y-6">
        ...
        {{-- Basic Information --}}
        <div class="rounded-2xl border p-6 space-y-5" style="background:var(--panel);border-color:var(--border)">
            <h3 class="font-semibold" style="color:var(--text-strong)">Basic Information</h3>

            <div>
                <label class="block text-sm font-medium mb-1.5" style="color:var(--text-soft)">Event Title *</label>
                <input type="text" name="title" value="{{ old('title', $event->title) }}" required
                       class="dark-input" placeholder="Event title">
            </div>

            <div>
                <label class="block text-sm font-medium mb-1.5" style="color:var(--text-soft)">Event Type *</label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium mb-1.5" style="color:var(--text-soft)">Start Date *</label>
                    <input type="date" name="start_date"
                           value="{{ old('start_date', $event->start_date->format('Y-m-d')) }}" required
                           class="dark-input">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1.5" style="color:var(--text-soft)">End Date *</label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium mb-1.5" style="color:var(--text-soft)">Start Time</label>
                    <input type="time" name="start_time" value="{{ old('start_time', $event->start_time) }}"
                           class="dark-input">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1.5" style="color:var(--text-soft)">End Time</label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium mb-1.5" style="color:var(--text-soft)">Capacity *</label>
                    <input type="number" name="capacity" value="{{ old('capacity', $event->capacity) }}" min="1" required
                           class="dark-input">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1.5" style="color:var(--text-soft)">Venue</label>
            ...
            <div>
                <label class="block text-sm font-medium mb-1.5" style="color:var(--text-soft)">Address</label>
                <input type="text" name="address" value="{{ old('address', $event->address) }}"
                       class="dark-input" placeholder="e.g. Phnom Penh, Cambodia">
            </div>

            <div>
                <label class="block text-sm font-medium mb-2" style="color:var(--text-soft)">Venue Setting *</label>
                <div class="grid grid-cols-3 gap-3">
                    ...
                    <label class="cursor-pointer">
                        ...
                        <div class="peer-checked:!border-indigo-500 peer-checked:!text-indigo-300 transition"
                             style="border:1px solid var(--input-border);border-radius:12px;padding:10px;font-size:13px;font-weight:500;text-align:center;color:var(--text-soft);background:var(--input-bg)">
                            {{ $label }}
                        </div>
                    </label>
                    @endforeach
            ...
            <div>
                <label class="block text-sm font-medium mb-1.5" style="color:var(--text-soft)">Description</label>
        ...
        <div class="rounded-2xl border p-6 space-y-5" style="background:var(--panel);border-color:var(--border)">
            <h3 class="font-semibold" style="color:var(--text-strong)">Event Settings</h3>

            {{-- Status --}}
            <div>
                <label class="block text-sm font-medium mb-1.5" style="color:var(--text-soft)">Status</label>
                <select name="status" class="dark-input" style="cursor:pointer">
            ...
            <label class="flex items-center gap-3 cursor-pointer">
                ...
                <div>
                    <p class="text-sm font-medium" style="color:var(--text-strong)">Public Event</p>
            ...
            <label class="flex items-center gap-3 cursor-pointer">
                ...
                <div>
                    <p class="text-sm font-medium" style="color:var(--text-strong)">Meal Provided</p>
                    <p class="text-xs" style="color:var(--text-soft)">Include catering in schedule and budget</p>
                </div>
            </label>
        </div>
